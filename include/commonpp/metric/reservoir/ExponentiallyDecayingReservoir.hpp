...
template <size_t s, size_t a, typename c>
ExponentiallyDecayingReservoir<s, a, c>::ExponentiallyDecayingReservoir()
: gen_(random_())
, dis_(0, 1)
{
...
template <size_t s, size_t a, typename c>
typename ExponentiallyDecayingReservoir<s, a, c>::Weight
ExponentiallyDecayingReservoir<s, a, c>::weight(ElapsedTimeUnit duration)
{
    return ::exp(ALPHA * duration.count());
}

template <size_t s, size_t a, typename c>
size_t ExponentiallyDecayingReservoir<s, a, c>::size() const
{
    std::lock_guard<thread::Spinlock> lock(mutex_);
    return values_.size();
}
...
template <size_t s, size_t a, typename c>
size_t ExponentiallyDecayingReservoir<s, a, c>::size_unsafe() const
{
    return values_.size();
}

template <size_t s, size_t a, typename c>
void ExponentiallyDecayingReservoir<s, a, c>::pushValue(double value,
                                                        Timepoint timestamp)
{
    std::lock_guard<thread::Spinlock> lock(mutex_);
    rescale();
...
template <size_t s, size_t a, typename c>
void ExponentiallyDecayingReservoir<s, a, c>::rescale()
{
